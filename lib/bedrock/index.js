...
module.exports = (data) => {
  // Set collisionShape on blocks based on block name for dynamic collision shapes
  function setBlockShapeTypes () {
    for (const block of data.blocksArray) {
      const name = block.name
      if (name.includes('_stairs')) {
        block.shapeType = 'stairs'
      } else if (name.includes('_fence') && !name.includes('_gate')) {
        block.shapeType = 'fence'
      } else if (name.includes('_pane') || name === 'iron_bars') {
        block.shapeType = 'pane'
      }
    }
  }
  ...
  function loadHashedRuntimeIds (registry) {
    ...
    for (const block of data.blocksArray) {
      for (const stateId of block.states) {
        const stateBlock = {...block, stateId}
        data.blocksByStateId[stateId] = stateBlock;
        data.blocksByRuntimeId[stateId] = stateBlock
      }
    }
    data.blockStates = buildIndexFromArray(data.blockStates, 'stateId')
  }
